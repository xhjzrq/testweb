        ...
        <el-main>
          <div class="zz" style="font-size:12px">
            ...
            <a-descriptions :title="one.titleName" bordered size="small" :column="4">
              ...
              <a-descriptions-item label="卡号" :span="2" />
            ...
            >
              ...
              <el-table-column
                ...
                align="center"
              />
              <el-table-column prop="bjgg" label="规格" align="center" />
              <el-table-column prop="th" label="标准图号" align="center" />

              <el-table-column prop="bjcz" label="材质" align="center" />
              <el-table-column prop="jldw" label="计量单位" align="center" />
              <el-table-column prop="zzsl" label="数量" align="center" />
              ...
              <el-table-column prop="cplx" label="类型" align="center" />
              <el-table-column prop="bjlc" label="来处" align="center" />
...
<script>
import { getDYList, getADy, getDYexcel } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    handleNodeClick(data) {
      console.log(data)
      const temp = data.czbh
      if (data.czbh.indexOf('-') == -1 || data.czbh == this.input) {
        return
      } else {
        this.zzth = data.czbh.substring(0, data.czbh.indexOf('-'))
        this.one.titleName1 = data.czbh.substring(data.czbh.indexOf('-') + 1, data.czbh.length)
      }
      ...
      getADy(this.input, this.zzth).then((res) => {
        ...
        this.one.titleName = res.data.list[0].kh
        this.one.ch = res.data.list[0].czbh
        this.one.qy = 'U' + res.data.list[0].kh.substring(6, 9)
        this.one.azth = res.data.list[0].azqy
        this.one.zzth = res.data.list[0].zzth
        this.one.tpbmc = this.one.titleName1
      }).catch(() => {
        this.one.ch = ''
        this.one.qy = ''
        this.one.azth = ''
    ...
    aaa() {
      getDYexcel(this.input, this.zzth).then((response) => {
        ...
        link.download = '单元生产设计数据.xlsx' // 下载的文件名
